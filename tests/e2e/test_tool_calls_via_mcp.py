...
async def mcp_session():
    """Fixture to spin up the MCP server subprocess and yield a ClientSession."""
...
async def test_search_indicator_tool(mcp_session: ClientSession):
...
async def test_get_indicator_data_tool(mcp_session: ClientSession):
...
async def test_get_country_indicator_tool_success(mcp_session: ClientSession):
...
async def test_get_country_indicator_tool_ambiguous(mcp_session: ClientSession):
...
async def test_export_report_tool(mcp_session: ClientSession, tmp_path):
