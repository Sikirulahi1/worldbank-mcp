...
This server exposes 4 powerful tools over standard I/O (`stdio`), allowing AI assistants like Claude to automatically search for, disambiguate, fetch, and export global economic indicators.
...
### Option 1: Claude Desktop (Production)
You do not need to run the server manually. Add the following to your Claude Desktop configuration file (e.g., `%APPDATA%\Claude\claude_desktop_config.json` on Windows):
